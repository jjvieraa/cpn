...
'fecha de comienzo y de finalización del ejercicio
    ...
    'SINO MesOperac=6, AñoOperc=2020, DiaVencePresupuesto=8 --> dFechaInicioEj = 9/5/2020
    ' dFechaFinEj = 8/6/2020
   If vpbMesOperac = 1 Then
            dFechaInicioEj = CDate(vpnPrspHst + 1 & "/12/" & vpnA�oOperac - 1)
    Else
            dFechaInicioEj = CDate(vpnPrspHst + 1 & "/" & vpbMesOperac - 1 & "/" & vpnA�oOperac)
    End If
    dFechaFinEj = CDate(vpnPrspHst & "/" & vpbMesOperac & "/" & vpnA�oOperac)
    HabilitaBotones
End Sub
...
'Cierra el mes
  ...
  Dim sMo As String
  sMo = "Contin�a el cierre ?" & vbCrLf & _
            "Si contin�a, se realizan las siguientes acciones: " & vbCrLf & _
    ...
    If Not fColocaMarcaAccion(26, vptMesPresup, "Cierre Comercio", "", "") Then
            MsgBox "Error 24233: No se complet� marca de Cierre"
    End If
    HabilitaBotones
    termina:
    mCierraTodo
